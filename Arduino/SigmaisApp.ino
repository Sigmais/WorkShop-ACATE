...
void setup() {
  ...
  while (flagInit == -1) {
    ...
    Serial.println("SIGMAIS"); // Make a clean restart
    delay(1000);
    PublicModeSF = 0;
    ...
    GetDeviceID();
    GetDevicePAC();
    GetAllowedTransmitionRepeatNumber();
...
void Send_Sensors(){
  ...
  Serial.print("Pressure BE: "); Serial.println(pressure.number);
  photo.number = (uint16_t) (tSensors->getPhoto() * 1000);
  Serial.print("Photo: "); Serial.println((float)photo.number/1000);
  Serial.print("Photo BE: "); Serial.println(photo.number);
  ...
  Serial.print("\0");
  Serial.print("Acc X BE: "); Serial.println(x_g.number);
  Serial.print("Acc Y BE: "); Serial.println(y_g.number);
  Serial.print("Acc Z BE: "); Serial.println(z_g.number);
  free(xyz_g);
...
void GetAllowedTransmitionRepeatNumber(){
  recvMsg *RecvMsg;
  const char msg[] = "AT$TR=?";
...
void GetVoltage(){
  ...
  RecvMsg = (recvMsg *)malloc(sizeof(recvMsg));
  Isigfox->sendMessage(msg, 5, RecvMsg);
...
void GetTemperature(){
  ...
  Serial.print("Temperature WISOL: ");
  for (int i=0; i<RecvMsg->len; i++){
    Serial.print(RecvMsg->inData[i]);
  }
  Serial.println("");
